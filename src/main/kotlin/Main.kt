...
fun App() {
    ...
    val copia by remember { mutableStateOf("nuevo") } //Variable que sirve para saber en que archivo lo pondra
    ...
    if (nuevoarchivo) {
        Dialog(
            onCloseRequest = { nuevoarchivo = false },
            onPreviewKeyEvent = {
                if (it.key == Key.Escape && it.type == KeyEventType.KeyDown) {
                    nuevoarchivo = false
                    true
            ...
            File(archivo).copyTo( //CopyTo coge primeramente el archivo que has seleccionado
                target = File(copia), //y luego pone el contenido del primer archivo dentro del archivo copia
                overwrite = true)
            val inputStream = File(copia).inputStream()
            val inputString = inputStream.bufferedReader().use { it.readText() }
            Text(inputString)
        }
    }
